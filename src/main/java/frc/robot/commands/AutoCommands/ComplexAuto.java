...
public class ComplexAuto extends SequentialCommandGroup {
  ...
  public ComplexAuto(Supplier<Double> xSteerSupplier, Supplier<Double> yDistanceSupplier, Supplier<Double> targetXSteerSupplier) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new TwoBallAuto(xSteerSupplier, yDistanceSupplier, targetXSteerSupplier),
      // new TurnGyro(60, -0.5),
      new RunCommand(()->{RobotContainer.mDrivetrain.setDrivetrain(-0.7, 0.7);}, RobotContainer.mDrivetrain).withTimeout(0.4),
